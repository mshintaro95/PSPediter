...
\begin{enumerate}
    \item 概要 \mbox{}\\
    数値$x$と$y$の集合の相関と有意性、$n$組のペアに対する回帰パラメータ\beta $_0$と\beta $_1$を計算し、与えられた見積もり値$x_k$を与えられた際の予測値$y_k$を計算し、それに対する70\%予測区間を計算する。
    ...
    \begin{enumerate}
        ...
        \[
        \sigma = \sqrt{(\frac{1}{n-2} \sum_{n = 1}^{n} (y_i - \beta _0 - \beta _1x_i)^2 )} 
        \]
    ...
    \begin{itemize}
        ...
        \item 入力ファイル：図\ref{data}のように、カンマ区切りの表形式で表現し、を持つ。\\
        ...
        \item 実行時入力例：java Program7 data\_program7.csv\\
